...
import { KANBAN_COLUMNS, normalizeStatus, statusToProgress } from "@/lib/kanban";
...
import DragOverlay from "@/components/kanban/DragOverlay";
...
export default function KanbanBoard({ projectId }: { projectId?: string }) {
  ...
  function applyFilters(input: KanbanTask[]) {
    let out = input;
  ...
  const filteredTasks = useMemo(() => applyFilters(tasks), [tasks, assigneeFilter, statusFilter, query, projectId]);
  ...
  async function loadAssigneeOptions(currentProjectId?: string) {

    // For performance, pull distinct assigned_to from tasks in scope.
    const scopeFilter = currentProjectId ? `.eq("project_id", '${currentProjectId}')` : "";
